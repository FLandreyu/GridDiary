...
const formRef = ref()
const form = ref({ username: '', password: '', confirm: '', email: '', nickname: '' })
const loading = ref(false)

const rules = {
  username: [
    { required: true, message: '请输入用户名', trigger: 'blur' },
    { pattern: /^[a-zA-Z0-9_]{3,50}$/, message: '3~50 位字母/数字/下划线', trigger: 'blur' }
  ],
  password: [{ required: true, min: 6, max: 50, message: '密码长度 6~50', trigger: 'blur' }],
  confirm: [
    { required: true, message: '请再次输入密码', trigger: 'blur' },
    {
      validator: (_r, v, cb) => (v === form.value.password ? cb() : cb(new Error('两次密码不一致'))),
      trigger: 'blur'
    }
  ],
  ...
  <el-card class="auth-card">
    ...
    <el-form ref="formRef" :model="form" :rules="rules" label-width="70px">
      ...
      <el-form-item label="密码" prop="password">
        <el-input v-model="form.password" type="password" show-password placeholder="6~50 位" />
      </el-form-item>
